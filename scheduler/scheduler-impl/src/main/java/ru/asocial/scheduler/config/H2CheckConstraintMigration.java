...
public class H2CheckConstraintMigration implements InitializingBean {
	...
	public void afterPropertiesSet() {
		...
		List<String> constraintNames = jdbcTemplate.query(
				"""
						...
						WHERE CONSTRAINT_TYPE = 'CHECK'
						  AND UPPER(TABLE_NAME) = 'TASK_SCHEDULES'
						""",
		...
		for (String constraintName : constraintNames) {
			...
			jdbcTemplate.execute("ALTER TABLE task_schedules DROP CONSTRAINT IF EXISTS \""
					+ constraintName.replace("\"", "") + "\"");
			log.info("Dropped H2 check constraint {} on task_schedules", constraintName);
		}
	}
