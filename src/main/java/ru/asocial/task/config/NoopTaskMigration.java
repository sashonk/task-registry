...
public class NoopTaskMigration {
	...
	public void removeNoopTasks() {
		int scheduleCount = jdbcTemplate.update("DELETE FROM task_schedules WHERE task_type = 'NOOP'");
		...
		if (scheduleCount + logCount + taskCount > 0) {
			log.info("Removed NOOP data: {} schedules, {} logs, {} tasks", scheduleCount, logCount, taskCount);
		}
	}
}
